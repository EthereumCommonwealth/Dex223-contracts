...
RevenueV1 stakes D223 (ERC-20 `0x675e…DD67` / ERC-223 `0x0908…80dd`) and its `factory` is `0xeA0A…`. Its `claim_delay` (the lock after each stake, for both unstaking and claiming) is the 10 day default. Do not `factory.setOwner(Revenue)`. No pools exist yet; once they do, the factory owner runs `scripts/revenue-enable-fees.ts`, then `scripts/revenue-collect.ts` with `REVENUE=0xbA75…`.
